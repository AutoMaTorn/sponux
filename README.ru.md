...
- **Калькулятор** — наберите выражение (`2+2*10`, `sqrt(2)`, `sin(pi/2)`) и
  ...
  `eval()`.
- **Поиск в интернете** — строка под локальными результатами отдаёт запрос
  браузеру, а запрос, который выглядит как адрес, предлагает его открыть. До
  нажатия Enter в сеть не уходит ничего: живых подсказок нет намеренно.
...
Ещё два — необязательные, и `.deb` просит их сам; это нужно только при запуске
из исходников или из тарбола:

```sh
sudo apt install libglib2.0-bin                  # gdbus: быстрое переключение
sudo apt install gir1.2-gtk4layershell-1.0       # только Wayland: размещение
```

`libglib2.0-bin` даёт `gdbus`, которым `bin/sponux` достаёт запущенный демон за
~11 мс вместо ~327 мс с импортом GTK. Без него ничего не ломается — просто
каждое открытие идёт медленным путём. `gir1.2-gtk4layershell-1.0` под X11 не
делает ничего; под Wayland именно он позволяет окну центрироваться и забирать
фокус. См. [Если что-то не так](docs/user-guide.ru.md#если-что-то-не-так).
...
| Клавиша | Действие |
| --- | --- |
| набор текста | поиск приложений / файлов / в интернете / вычисления |
...
Префиксы сужают поиск: `f:` файлы, `a:` приложения, `c:` или `=` калькулятор,
`?` интернет, а ведущий `/` или `~/` дополняет путь вместо поиска. И клавиши, и
префиксы показаны в самом окне, так что запоминать ничего не нужно.
...
заготовки с комментариями. `[window]` задаёт ширину карточки, число результатов
и размещение; `[web]` выбирает поисковую систему; `[keys]` переназначает
действия на модификаторах. Правки применяются при следующем открытии, без
перезапуска.
...
```sh
python3 tools/mkdeb.py         # dist/sponux_0.3.0-1_all.deb   (нужен dpkg-deb)
python3 tools/mktarball.py     # dist/sponux-0.3.0.tar.gz      (распаковать и запускать)
...
python3 tests/test_web.py         # поисковые URL, что считается адресом, место строки
